...
public class HomeController {
	...
	public String callback(Model model, @RequestParam String code, @RequestParam String state, HttpSession session, RedirectAttributes rdat)
	    ...
	    //위에 jsp로 변수값 보내는 로직을 타지않고, 스프링시큐리티로 로직을 탈거임.
	    
	    /* Json 데이터 파싱 시작 */
	    /** apiResult의 데이터 형태는 아래와 같음.
	    {"resultcode":"00",
	    "message":"success",
	    ...
	    **/
	    ...
	    //Top레벨 단계 _response 파싱(아래)
	    //파싱 2번 하는 이유: 위 데이터값이 배열안에 배열이 있어서
	    JSONObject response_obj = (JSONObject)jsonObj.get("response");
	    //파싱 마지막인 response 객체의 name값과 email값 뽑기(아래)
	    ...
	    if(Status.equals("success")) {
	    	...
	    	//네이버로 SNS로 로그인한 사람들은 강제로 스프링 시큐리티 인증처리함 아래
		    List<SimpleGrantedAuthority> authorities = new ArrayList<>();
		    //강제 로그인한 사람은 ROLE_USER 부여
			...
			rdat.addFlashAttribute("msg", "SNS 방식의 로그인");//result msg데이터를 숨겨서 jsp로 전송
	    } else {
	    	//즉, 네이버 로그인이 실패했을때(아래)
		...
		return "redirect:/";//새로고침시 자동 등록 방지를 위해서
	}
	
	/**
	 * 로그인 페이지 파일 입니다. 200908 수정
	 */
	@RequestMapping(value = "/login", method = RequestMethod.GET)
	public String login(Locale locale, Model model, HttpSession session) {
		
		/* 네이버아이디로 인증 URL을 생성하기 위하여 
		 * naverLoginBO클래스의 getAuthorizationUrl메소드 호출
		 * session은 전여변수와 같은역할을 함.
		 * anverAuthIrl은 http://127.0.0.1:8080/login이고,
		 * 이 값을 네이버 API에서 가져오는 메소드가 getAuthorizationUrl(session) */
